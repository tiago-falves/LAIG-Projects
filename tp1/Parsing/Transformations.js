    ...
    getTransformationMatrix(transformations, transformationIndex){
        ...
        for (let j = 0; j < transformations.length; j++) {
            ...
            switch(transformations[j].nodeName){
                case "translate":
                    coordinates = this.scene.parseCoordinates3D(transformations[j], "translate transformation for ID " + transformationIndex);
                    matrix_transformation = mat4.translate(matrix_transformation, matrix_transformation, coordinates);
                    break;
                case "scale":
                    coordinates = this.scene.parseCoordinates3D(transformations[j], "scale transformation for ID " + transformationIndex);
                    matrix_transformation = mat4.scale(matrix_transformation, matrix_transformation, coordinates);
                    break;
                    
                case "rotate":
                    let axis = this.scene.reader.getString(transformations[j], 'axis');
                    let angle = this.scene.reader.getFloat(transformations[j], 'angle'); 
                    let vector;
                ...
                case "transformationref":
                    let transfRef = this.scene.reader.getString(transformations[j],'id');
                    if (this.scene.transformations[transfRef] == null) {
                        return "Unable to get transformation reference";
                    }
                    matrix_transformation = mat4.multiply(matrix_transformation,matrix_transformation,this.scene.transformations[transfRef]);
                    break;
            }
        }
